    ...
    .post((req,res)=>{
     ...
     const sql ='SELECT * FROM MENU';
          ...
          if (result.length>0){
            ...
            let result={'status' :"Success", "msg" : "OK","data":"No hay datos"};
            res.json(result);
...
router.route('/CudMenu').post((req,res)=>{
  ...
  let sql = `UPDATE MENU
    ...
    const data={
      ...
      connection.query(sql, data, (error, results, fields) => {
        if (error){
          let result={'status' :"NoSuccess", "msg" : "ERROR","data":"error en el guardado!!!"};
          res.json(result);
        }
//        console.log('Rows affected:', results.affectedRows);

        let result={'status' :"Success", "msg" : "OK","data":"Datos guardados correctamente"};
        res.json(result);
